...
def Random_Player(player, game, weights = None):
    """
...
# Identical to Minimax_Player, except weights[0] and weights[1] vary.
def Smart_Minimax_Player(player, game, weights):
    WEIGHT_DELTA = 0.1
    weights[0] -= WEIGHT_DELTA
    # takes in a player and a board, and updates the player's corners depending on the state of the board
    def check_corners(player, board):
        """
            Updates the corners of the player in the test board (copy), in case the
...
# weights[1] determines how important maximizing the difference of my corners and opponent corners
# weights[2] decides how many of the best placements we choose to look ahead with
# weights[3] decides the number of random trials for each piece
# weights[4] decides how deep will each trial go

def Smart_Random_Player(player, game, weights):
	"""
	Randomly pick the piece multiple times to find out which move results in the most win.
	Pick such move.
	Note that the opponents also move randomly.
	"""
	TRIALS = weights[3]

	def random_trials(player, game):
		print("Inside random trials")
		game_copy = copy.deepcopy(game)
		for p in game_copy.players:
			p.strategy = Random_Player
		
		# counts number of iterations, and stop if count reaches (3 + 4*weights[4]) iteration 
		# Note that we do (3 + 4*weights[4]) to ensure that every player has equal turns.
		count = 0
		max_rounds = 3 + 4*weights[4]
		while game_copy.winner() == "None" and count < max_rounds:
			game_copy.play()
			count += 1

		print("Exitting random trials")
		# If game reached the end before (3 + 4*weights[4]) iteration
		if game_copy.winner() != "None":
			return game_copy.winner() 
		# If not, return a player with the highest score.
		else:
			players = [(p.score, p.name) for p in game_copy.players]
			print(players)
			return(sorted(players, reverse=True)[0][1])

	# creat a copy of the player's pieces
	shape_options = [p for p in player.pieces]
	# determine all possible moves
	possibles = player.possible_moves(shape_options, game)
	
	# final_piece will be returned.
	final_piece = None
	...
	if possibles != []:
		...
		# take at most the n highest scoring moves
		if len(by_score) > weights[2]:
			possibles = by_score[:weights[2]]
		else:
			possibles = by_score

		##### ROLLOUT GAME ######
		final_piece = possibles[0][0]
		max_win_num = -1
		for piece in possibles:
			...
			for i in range(TRIALS):
				print("####### Trial %d #######" % (i+1))
				game_copy = copy.deepcopy(game)
				current = [p for p in game_copy.players if p.name == player.name][0]

				# ensure that the move is valid
				if game_copy.valid_move(current, piece.points):
					# update the board with the move
					game_copy.board.update(current, piece.points)
					# let the player update itself accordingly
					current.update_player(piece, game_copy.board)
					# remove the piece that was played from the player
					current.remove_piece(piece)
					# place the player at the back of the queue
					first = game_copy.players.pop(0)
					game_copy.players = game_copy.players + [first]
					# increment the number of rounds just played
					game_copy.rounds += 1
				else: raise Exception("Invalid move by " + current.name + ".")
				...
				winner = random_trials(player, game_copy)
				print("##### winner is %s #####\n\n\n" %(winner))
				if winner == current.name:
					cur_win_num += 1
			# update if cur_win_num is greater than max_win_num.
			if cur_win_num > max_win_num:
				print("###!!! Update final piece !!!###")
				max_win_num = cur_win_num
				final_piece = piece

			print("####### For piece %s: winning number = %d #######\n\n\n" % (piece.ID, cur_win_num))
	
	# return either some piece or None.
	return final_piece
